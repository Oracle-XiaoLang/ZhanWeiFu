...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //银行卡
    NSLog(@"dffffffff%@",[defaults3 objectForKey:@"YinHangKaXinXi_YinHangKaHao_Field"]);
    ...
    //照片上传
    NSLog(@"vvvv%@",[defaults3 objectForKey:@"zjz1"]);
...
- (void)viewDidLoad {
   ...
   // _DataIntegrity.backgroundColor=[UIColor blackColor];
    ...
    _imagelianxiren=[[UIImageView alloc] initWithFrame:CGRectMake((304-110)/2, 197, 110, 113)];
    ...
    _imageyinhangka=[[UIImageView alloc] initWithFrame:CGRectMake(5, 95, 110, 113)];
    ...
    _imageziliao=[[UIImageView alloc] initWithFrame:CGRectMake(304-115, 95, 110, 113)];
    ...
    _image=[[UIImageView alloc] initWithFrame:CGRectMake((304-110)/2, 2, 110, 113)];
...
- (void)YinHangkaTiaoZhuan {
   ...
   if ([[defaults3 objectForKey:@"LianXiRenXinXi_Name_Field"] isEqual:@""]||[[defaults3 objectForKey:@"LianXiRenXinXi_Name_Field"]isEqual:@""]) {
        HUD = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
        
        HUD.mode = MBProgressHUDModeText;
        
        HUD.labelText = @"请先填写完联系人信息在进入";
